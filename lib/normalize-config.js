...
// Returns a validated and normalized Stickler configuration.
function normalizeConfig(config = {}, configDir) {
  ...
  if (Array.isArray(normalized.jsLint)) {
    ...
    normalized.jsLint.forEach((jsLintItem, index) => {
      ...
      jsLintItem.files = dirGlob.sync(
        jsLintItem.files.map(absolutePath(configDir))
      );
    });
    if (!hasFallback) {
